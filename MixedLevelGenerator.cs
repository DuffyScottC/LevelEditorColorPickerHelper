...
public class MixedLevelGenerator : MonoBehaviour {
	...
	[Header("GO Player Entities")]
	public GOEntity[] gOPlayerEntities = new GOEntity[] {
		new GOEntity("Player 1", new Color32(0, 0, 255, 255))
	};
	[Tooltip("Must have the same size as gOPlayerEntities")]
	public GameObject[] gOPlayerGameObjects;


	[Header("Tilemap:")]

	[Header("T Terrain Entities")]
	public TEntity[] tTerrainEntities = new TEntity[] {
		new TEntity("Barrier", new Color32(124, 107, 68, 255)),
		new TEntity("Barrier Breakable", new Color32(216, 177, 139, 255)),
		new TEntity("Barrier Hole", new Color32(41, 38, 36, 255)),
		new TEntity("Ground", new Color32(228, 206, 156, 255)),
	};
	[Tooltip("Must have the same size as tTerrainEntities")]
	...
	public void Start() {
		...
		for (int i = 0; i < tTerrainTileBases.Length; i++) {
			tTerrainEntities[i].tile = tTerrainTileBases[i];
		}
